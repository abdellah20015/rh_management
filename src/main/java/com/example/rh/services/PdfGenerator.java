...
public class PdfGenerator extends AbstractVerticle {
  ...
  private void generatePdfHandler(Message message) {
    ...
    try {
      String pdfFilePath = data.getString("type") + "_report_" + System.currentTimeMillis() + ".pdf"; ;
      ...
      message.reply("PDF generated successfully at " + pdfFilePath);
    }catch (Exception e) {
      System.out.println(e);
    }
  }
  ...
  private void addContent(Document document, JsonObject data) throws DocumentException {
    ...
    preface.add(new Paragraph("A: " + data.getString("username"), new Font(Font.FontFamily.HELVETICA, 14, Font.NORMAL, BaseColor.BLACK)));
    preface.add(new Paragraph("de: " + data.getString("manager_username"), new Font(Font.FontFamily.HELVETICA, 14, Font.NORMAL, BaseColor.BLACK)));
    preface.add(new Paragraph("date: " + data.getString("created_date"), new Font(Font.FontFamily.HELVETICA, 14, Font.NORMAL, BaseColor.BLACK)));
