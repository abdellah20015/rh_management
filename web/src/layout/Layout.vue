<template>
  <div class="flex flex-col min-h-screen bg-gray-50">
    <!-- Navbar Modern -->
    <nav class=" shadow-md border-b border-gray-100 bg-white">
      <div class="container mx-auto px-6">
        <div class="flex items-center justify-between h-16">
          <!-- Logo à gauche -->
          <div class="flex-shrink-0">
            <div class="font-semibold text-[#006aff]">
              <RouterLink :to="{ name: 'statistics' }">
                <img src="../asset/RH.png" alt="Logo" width="100px" />
              </RouterLink>
            </div>
          </div>
          ...
          <div class="flex-1 flex justify-center">
            <div class=" md:flex items-center space-x-8">
              ...
              <RouterLink v-if="user.role != 'admin'" :to="{ name: 'userDemands' }"
                class="text-[#6c7f93] hover:text-[#006aff] font-medium transition-all duration-200 text-sm">
                Demandes
              </RouterLink>
            </div>
          ...
          <div class="flex items-center space-x-4">
            <p class="text-[#006aff] bg-[#c6dffb] py-1 px-3 rounded-[6px] font-semibold text-sm">
            ...
            <div class="relative">
              <button @click="toggleNotificationDropdown"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7  text-[#6c7f93]" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                <span v-if="notifications.map(notifi => notifi.is_read).includes(false)"
                  class="absolute top-1 right-1 bg-[#FA5252] text-white text-xs rounded-full w-3 h-3 flex items-center justify-center">
              ...
              <transition name="fade-scale">
                <div v-if="isNotificationDropdownOpen"
                  ...
                  <div class="max-h-80 overflow-y-auto">
                    <div v-if="notifications.length === 0" class="p-4 text-center text-[#6c7f93]">
                        ...
                        <div class="flex-shrink-0">
                          <div class="w-8 h-8 rounded-full flex items-center justify-center"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                              :class="notification.is_read ? 'text-[#6c7f93]' : 'text-[#006aff]'" viewBox="0 0 20 20"
                              fill="currentColor">
                              <path
            ...
            <div class="relative">
              <button @click="toggleProfileDropdown"
                class="flex items-center justify-center rounded-full p-2 hover:bg-[#99c4ff]/20 transition-colors duration-200">
                <img src="https://img.icons8.com/office/40/test-account.png" alt="Profile"
                  ...
                  <div class="py-1">
                    <button @click="profile"
                      ...
                      <div class="flex gap-3">
                        <img width="20" height="20" src="https://img.icons8.com/offices/30/gender-neutral-user.png"
    ...
    async updateNotificationStatus(notification) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          } else if (this.user.role == "admin" && this.user.role == "manager") {
            router.push({ "name": "TeamDemands" });
          }
        }
      } catch (err) {
        console.error(err);
